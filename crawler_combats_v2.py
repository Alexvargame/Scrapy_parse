...
def getLinks(articleUrl):
    ...
    priem=['Выстрел в голову','Кристаллизации','Ядовитого Взрыва','Колотых ран','Магической раны','Двойной бросок','Живой Искры','Мясорубка','Глубоких порезов','Броском топора','Оледенения','Отравления','Ядовитого Пузыря','Метеорита','Огненному Щиту','Астральный удар','Пожирающего Пламени']#,'Сгусток чудовищной энергии']
        ...
        for key, value in ldict.items():
            for v in value:
                ...
                if re.findall(r'\d+\/\d+',v) and v.find('Мана')==-1 and v.find('Исцеление')==-1 and v.find('регенерирует здоровье')==-1 and not re.findall(r'\s+--',v) and not re.findall(r'\.\s+--',v) and not re.findall('|'.join(health),v):
                    ...
                    if len(list(re.finditer(regex,v)))==3:
                        print('3')
##                        
                        if re.findall('|'.join(priem),v):
                          
                            if list(re.finditer(regex,v))[2][0][1:].strip()[0]=='-':                                
                                result[list(re.finditer(regex,v))[0][0]][list(re.finditer(regex,v))[1][0]]+=int(list(re.finditer(regex,v))[2][0][1:].strip()[1:])                                 
                            else:
                                result[list(re.finditer(regex,v))[1][0]][list(re.finditer(regex,v))[0][0]]+=int(list(re.finditer(regex,v))[2][0][1:].strip()[1:])
                            ...
                            if list(re.finditer(regex,v))[2][0][1:].strip()[0]=='-':
                                ...
                                result[list(re.finditer(regex,v))[0][0]][list(re.finditer(regex,v))[1][0]]+=int(list(re.finditer(regex,v))[2][0][1:].strip()[1:])
                        print('temp',result)
                    elif len(list(re.finditer(regex,v)))==2:
                        if (list(re.finditer(regex,v))[0][0] not in (list(reduce(lambda x,y:x+y,[v for v in team.values()])))) or (list(re.finditer(regex,v))[1][0] not in (list(reduce(lambda x,y:x+y,[v for v in team.values()])))):
                            print('2')
                            
                            if len([p for p in priem if v.find(p)!=-1])>0:
                                print('PR')
                                sleep(3)
                                #uron=int(list(re.finditer(regex,v))[1][0][1:].strip()[1:])
                                #int(list(re.findall(r'\.\s+.\d+',v))[0].strip()[1:])
                                #print((re.findall(r'\s+.\d+',v))[0].strip())
                                for bs_t in bs_time:
                                    #print(bs_t)
                                    ...
                                    #if a.name and a.get_text() and b.name and b.b:
                                        
                                        #print('a',a,a.name,a.get_text(),'111','b',b,b.name,b.b.get_text(),'BOOL',b.b.get_text()==list(re.finditer(regex,v))[1][0][1:].strip(),a.get_text()==[p for p in priem if v.find(p)!=-1][0])
                                        #input()
                                    if a.name=='b' and a.get_text()==[p for p in priem if v.find(p)!=-1][0]:
                                        print(a)
                                        if b.name=='font' and 'title' in b.attrs and b.b.get_text()==list(re.finditer(regex,v))[1][0][1:].strip():
                                            #print('attrs',b.attrs,'b_uron',b.b.get_text())
                                            #input()
                                            bs_font=b
                                            print('bs',bs_font)
                                            break
##                                for sib in key.next_siblings:
##                                    if sib.name=='b' and sib.get_text()==[p for p in priem if v.find(p)!=-1][0] and v.find(sib.get_text())!=-1:
##                                        if sib.next_sibling.next_sibling.name=='font' and 'title' in sib.next_sibling.next_sibling.attrs:
##                                                bs_font=sib.next_sibling.next_sibling
##                                                print('BS',bs_font)
##                                                break                     
                            else:
                                #input()
                                ...
                                for bs_t in bs_time:
                                    #print(bs_t)
                                    #print(bs_t.next_sibling,bs_t.next_sibling.next_sibling,bs_t.next_sibling.next_sibling.next_sibling.next_sibling)
                                    a=bs_t.next_sibling.next_sibling.next_sibling.next_sibling
                                    #b=bs_t.next_sibling.next_sibling.next_sibling.next_sibling.next_sibling.next_sibling
                                    #print('a',a,'b',b,)
                                    if a.name=='font' and 'title' in a.attrs and a.b.get_text()==list(re.finditer(regex,v))[1][0][1:].strip():
                                        #print('attrs',b.attrs,'b_uron',b.b.get_text())
                                        #input()
                                        bs_font=a
                                        print('bs',bs_font)
                            ...
                            if list(re.finditer(regex,v))[1][0][1:].strip()[0]=='-':
                                
                                result[bs_font['title'][bs_font['title'].find('b')+2:bs_font['title'].find('/b')-1]][list(re.finditer(regex,v))[0][0]]+=int(list(re.finditer(regex,v))[1][0][1:].strip()[1:])
                            else:
                                result[list(re.finditer(regex,v))[0][0]][bs_font['title'][bs_font['title'].find('b')+2:bs_font['title'].find('/b')-1]]+=int(list(re.finditer(regex,v))[1][0][1:].strip()[1:])
                       
                            print('temp',result)
                            
                        else:
                            if v.find('казнил')!=-1:
                                result[list(re.finditer(regex,v))[0][0]][list(re.finditer(regex,v))[1][0]]+=int(list(re.finditer(r'\s+.\d+',v))[0][0][1:].strip()[1:])
                                print('2-res',result)
                                
                    elif len(list(re.finditer(regex,v)))==1:
                        print('1')
                        #uron=int(list(re.findall(r'\s+.\d+',v))[0].strip()[1:])
                        #print((re.findall(r'\s+.\d+',v))[0].strip())
                        for bs_t in bs_time:
                            #print(bs_t)
                            a=bs_t.next_sibling.next_sibling.next_sibling.next_sibling
                            ...
                            if a.name=='b' and a.get_text()==[p for p in priem if v.find(p)!=-1][0]:
                                if b.name=='font' and 'title' in b.attrs and b.b.get_text()==re.findall(r'\s+.\d+',v)[0].strip():
                                    print('attrs',b.attrs,'b_uron',b.b.get_text())
                                    #print('2',sib.next_sibling.next_sibling,'2N',sib.next_sibling.next_sibling.name,'2N',sib.next_sibling.next_sibling.get_text(),'4',sib.next_sibling.next_sibling.next_sibling.next_sibling)
                                    #input()
                                    bs_font=b
                                    print('bs',bs_font)
                                    break
                        ...
                        if list(re.findall(r'\s+.\d+',v))[0].strip()[0]=='-':
                            result[bs_font['title'][bs_font['title'].find('b')+2:bs_font['title'].find('/b')-1]][list(re.finditer(regex,v))[0][0]]+=int(list(re.findall(r'\s+.\d+',v))[0].strip()[1:])
                        else:
                            result[list(re.finditer(regex,v))[0][0]][bs_font['title'][bs_font['title'].find('b')+2:bs_font['title'].find('/b')-1]]+=int(list(re.findall(r'\s+.\d+',v))[0].strip()[1:])
                        print('2-res',result)
